...
wss.on("connection", (client, req) => {
    ...
    agent.on("message", async (buf) => {
        ...
        //     console.log("agent_response==>", json);
        ...
        if (json?.type === "agent_response") {
            const closingText = json?.agent_response_event?.agent_response || "";

            console.log("closingText==>", closingText);

            if (isClosingMessage(closingText)) {
                console.log("🔚 AGENT THANKYOU DETECTED → SERVER CLOSING");
    ...
    agent.on("close", () => {
        if (!conversationState.finalClosed) {
            console.log("🤖 AGENT WS CLOSED → FORCE FINALIZE");
